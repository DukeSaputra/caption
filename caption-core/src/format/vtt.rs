...
mod tests {
    ...
    fn starts_with_webvtt_header() {
        let words = vec![word("hello", 0.5, 0.82)];
        let config = FormatConfig::default();
        let vtt = VttFormatter.format(&words, &config).unwrap();
        assert!(
            vtt.starts_with("WEBVTT\n\n"),
            "VTT must start with WEBVTT header followed by blank line"
        );
    }
    ...
    fn timestamp_uses_period_not_comma() {
        ...
        assert!(ts.contains('.'), "VTT timestamps must use period: {ts}");
        assert!(
            !ts.contains(','),
            "VTT timestamps must NOT use comma separator: {ts}"
        );
    }
    ...
    fn overlapping_timestamps_clamped() {
        ...
        assert!(vtt.contains("00:00:00.000 --> 00:00:00.499"));
    }
    ...
    fn min_cue_duration_enforced() {
        ...
        assert!(vtt.contains("00:00:01.000 --> 00:00:01.200"));
    }
    ...
    fn min_cue_duration_after_overlap_clamp() {
        ...
        assert!(vtt.contains("00:00:00.000 --> 00:00:00.049"));
    }
    ...
    #[test]
    fn two_words_full_output() {
        let words = vec![word("Hello", 0.5, 0.82), word("world", 0.82, 1.15)];
        let config = FormatConfig::default();
        let vtt = VttFormatter.format(&words, &config).unwrap();

        let expected = "WEBVTT\n\n\
            1\n00:00:00.500 --> 00:00:00.820\nHello\n\n\
            2\n00:00:00.820 --> 00:00:01.150\nworld\n\n";
    ...
    fn no_overlapping_cue_ranges() {
        ...
        let timestamps: Vec<(f64, f64)> = body
            .split("\n\n")
    ...
    fn one_hour_video_timestamps() {
        ...
        let config = FormatConfig::default();
        let vtt = VttFormatter.format(&words, &config).unwrap();
        assert!(vtt.contains("01:01:01.500 --> 01:01:02.000"));
    }
    ...
    #[test]
    fn file_extension_is_vtt() {
        assert_eq!(VttFormatter.file_extension(), "vtt");
    }
